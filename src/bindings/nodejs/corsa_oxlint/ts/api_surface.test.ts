  ...
  it(
    ...
    () => {
      ...
      writeFileSync(
        ...
        [
          `import { OxlintUtils, RuleCreator, definePlugin, defineRule } from "${importPath}";`,
          "",
          ...
          "export const inferredRule = createRule({",
          '  name: "inferred",',
          '  meta: { type: "problem", docs: { description: "d" }, messages: { m: "m" }, schema: [] },',
          "  defaultOptions: [],",
          "  create(context) {",
          "    return {",
          "      NewExpression(node) {",
          '        if (node.callee) context.report({ node, messageId: "m" });',
          "      },",
          "    };",
          "  },",
          "});",
