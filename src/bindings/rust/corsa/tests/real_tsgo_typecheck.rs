...
#[test]
fn real_tsgo_handles_monorepo_project_references_and_strict_config() {
    block_on(async {
        let Some(binary) = support::resolved_real_tsgo_binary() else {
            return;
        };
        ...
        for mode in [ApiMode::SyncMsgpackStdio, ApiMode::AsyncJsonRpcStdio] {
                ...
                .file
            {
                ...
                let error = client
                    ...
                    .unwrap_err();
                assert!(
                    matches!(error, TsgoError::Unsupported(_)),
                    "expected unsupported diagnostics for current real tsgo in {mode:?}, got {error:?}"
                );
            }
